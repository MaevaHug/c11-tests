...
cc -Wall -Wextra -Werror -o ft_count_if ft_count_if.c main.c
if [ $? -ne 0 ]; then
...
longest_test="Running tests for \`ft_count_if\` function with \`str_is_numeric\` as the callback function."
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_count_if $(format_args "${args[@]}")"

	output=$(./ft_count_if "${args[@]}")
...
echo "$separator"
echo "Running tests for \`ft_count_if\` function with \`str_is_numeric\` as the callback function."
echo "The function will iterate over the array and count how many elements are numeric."
...
run_test 123 456 789 "3" || all_tests_passed=false
...
run_test 123 abc 456 def "2" || all_tests_passed=false
# Test: Argument "NULL"
