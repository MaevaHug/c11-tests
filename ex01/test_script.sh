...
cc -Wall -Wextra -Werror -o ft_map ft_map.c main.c
if [ $? -ne 0 ]; then
...
longest_test="Running tests for ft_map function with \`multiply_by_two\` as the callback function."
...
echo "$separator"
echo "Running tests for ft_map function with \`multiply_by_two\` as the callback function."
echo "The function will iterate over the array and will multiply by 2 each element."

# Test: Single integer
run_test 42 "84 " || all_tests_passed=false
# Test: Multiple integers
run_test 1 2 3 4 5 "2 4 6 8 10 " || all_tests_passed=false
# Test: Negative integers
run_test -1 -2 -3 -4 -5 "-2 -4 -6 -8 -10 " || all_tests_passed=false
# Test: Mixed integers
run_test 0 -1 2 -3 4 "0 -2 4 -6 8 " || all_tests_passed=false
